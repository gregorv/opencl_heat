...
__kernel void solve_heat_equation(__global float* iTemperature, __global float* oTemperature, __global float2* gradientField, int2 resolution, __write_only image2d_t dest)
{
	...
	int ym = y<=0? y : y-1;
	float div = (gradientField[xp*resolution.y + y].x - gradientField[xm*resolution.y + y].x
			   + gradientField[x*resolution.y + yp].y - gradientField[x*resolution.y + ym].y)*0.5f;
	oTemperature[x*resolution.y + y] = iTemperature[x*resolution.y + y] + div*0.3f;
	*/
